...
/**
 * Logo "G" nhiều màu của Google — dùng riêng cho thẻ đánh giá Google, không
 * đi qua lcdc_dental_icon() vì icon đó luôn vẽ 1 màu (currentColor/fill đặc).
 *
 * @param string $classes Class CSS bổ sung cho thẻ <svg>.
 * @return string HTML <svg> đã an toàn (không cần esc thêm ở nơi gọi).
 */
if (!function_exists('lcdc_google_icon')) {
    function lcdc_google_icon(string $classes = ''): string
    {
        $cls = trim('lcdc-icon lcdc-icon--google ' . $classes);

        return '<svg class="' . esc_attr($cls) . '" width="1em" height="1em" style="width:1em;height:1em;display:inline-block;vertical-align:middle;" viewBox="0 0 48 48" aria-hidden="true" focusable="false">'
